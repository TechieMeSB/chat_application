...
class GUI:
    ...
    def receive_message_from_server(self, so):
        while True:
            try:
                buffer = so.recv(256) #get a buffer to reads whatever data the client sends and echoes it back
                if not buffer:
                    break
    ...
    def encrypt(self):
        if len(self.name.get()) == 0:
            messagebox.showerror("Enter your message", "Enter your message to be Encrypted")
            return
        ...
        senders_name =self.name_widget.get()+ ": Can you decrypt this :-  "
